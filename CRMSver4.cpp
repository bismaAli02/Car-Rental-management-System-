...
void cust_display();
void models();
void invoice(string car_name, int day, int rental_amount);
...
main()
{
  ...
  int budget = 0, rental_amount = 0, rent = 0, cust_id = 0, day, Advance = 0;
  string cust_name, cust_p, car_name;
    ...
    else if (status == 'b')
    {
      if (checkUser(id, password))
      {
...

        {
          ...
          {
            invoice(car_name, day, rental_amount);
            system_cls();
          } // bill of customer
          else if (option == '5')
          {
...
void invoice(string car_name, int day, int rental_amount)
{
  ...
  for (int i = 0; i < car_count; i++)
  {
    ...
    {
      cout << "If you avail this car more than 4 days you will get 5% discount" << endl;
      ...
      cout << "YOUR TOTAL AMOUNT: " << rental_amount << endl;
    }
  }
  if (rental_amount == 0)
  {
...
void add_C()
{
  ...
  for (int i = 0; i < count; i++)
  {
    if (cust_idA[i] == c_id)
    {
      cout << "You already assign this id to another customer Use another id" << endl;
      n = 1;
      break;
    }
  }
